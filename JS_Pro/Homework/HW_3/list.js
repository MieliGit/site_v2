...
for (let i = 0; i < localStorage.length; i++) {
  ...
  divProduct.className = "div-reviews";
  listProducts.insertAdjacentElement("beforeend", divProduct);
  divProduct.insertAdjacentHTML(
    "beforeend",
    `<h2>${product} <button class="button-open" onclick="openReview(this)">Развернуть</button> </h2>`
  );
  ...
  reviewSet.forEach((element) => {
    ...
    newReview.className = "div-reviews";
    const reviewText = document.createElement("span");
    reviewText.className = "text-span";
